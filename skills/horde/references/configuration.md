...
`horde config` prints the complete default TOML. Settings merge in this order,
later winning:
...
A role is a name a template step asks for. Configure each role you use.
...
Preconfigured roles out of the box: `planner`, `worker`, `reviewer` (Codex),
`native` (Tuara), `claude` (Claude Code), `simulated`.
...
Model names for the harness kinds inherit their provider default unless set
...
**Subscription login (default, `auth_mode = "login"`).** Codex and Claude use their
own installed CLI and existing credential store. Horde does not read or copy those
credentials.

**API keys (`auth_mode = "api"`).** Set `base_url` and `api_key_env`:
